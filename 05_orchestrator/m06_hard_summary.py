"""Build M06 hard 6-test matrix summary."""
...
def write_hard_summary(run_id: str, run_root: Path, results: List[Dict[str, Any]], project: str) -> Dict[str, Any]:
    ...
    nav_required = [r for r in results if r.get("navigation_required")]
    nav_used = [r for r in results if r.get("navigation_used")]
    activities_after_nav = [
        r
        for r in results
        if r.get("navigation_used") and r.get("m06_status") in ("PASS", "PASS_ALREADY_IN_ACTIVITIES")
    ]
    ...
    if pollution or full_screen_ocr:
        fixes.append("Audit M06 capture paths for P6-only OCR")
    if false_pass:
        fixes.append("Tighten M06 workspace detection and navigation gates")
    if unsafe_action:
        fixes.append("Remove forbidden keyboard actions from M06 steps")
    if not fixes:
        fixes.append("Monitor with TY_TEST_M06_HARD_6.bat")
    ...
    decision = "M06 STABLE" if stable else "M06 NEEDS FIX"
    next_rec = "READY FOR NEXT MODULE" if stable else "FIX M06 AGAIN"
    ...
    summary = {
        ...
        "navigation_required_tests": [r.get("test_id") for r in nav_required],
        "navigation_used_tests": [r.get("test_id") for r in nav_used],
        "activities_confirmed_after_navigation_tests": [r.get("test_id") for r in activities_after_nav],
        "top_issues": problems[:3],
        "fixes_applied": fixes[:3],
        "tests": results,
    }

    json_path = run_root / "m06_hard_test_6_summary.json"
    with json_path.open("w", encoding="utf-8") as handle:
        json.dump(summary, handle, indent=2, ensure_ascii=False)

    lines = [
        "# M06 HARD TESTING SUMMARY",
        "",
        f"Run ID: {run_id}",
        f"Project: {project}",
        "Total tests: 6",
    ...
    for r in results:
        lines.append(
            ...
            f"{r.get('m06_status')} (score {r.get('score')}) — {r.get('score_reason', '')}"
        )
    ...
    lines.extend(
        [
            ...
            "",
            f"- Tests where Alt+P, A was required: {', '.join(summary['navigation_required_tests']) or '(none)'}",
            f"- Tests where Alt+P, A was used: {', '.join(summary['navigation_used_tests']) or '(none)'}",
            f"- Tests where Activities was confirmed after navigation: "
            f"{', '.join(summary['activities_confirmed_after_navigation_tests']) or '(none)'}",
            "",
            "Top issues:",
        ]
    )
    ...
    lines.extend(
        [
            ...
            "Evidence:",
            str(json_path),
            str(run_root / "m06_hard_test_6_summary.md"),
        ]
    )

    md_path = run_root / "m06_hard_test_6_summary.md"
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return summary
