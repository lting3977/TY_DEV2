...
from export_wizard_common import m20_hard_dismiss_stale_dialogs  # noqa: E402
...
TEST_06_OK = frozenset({"FAIL_ACTIVITIES_NEXT_SCREEN_NOT_FOUND"})
...
def score_result(
    ...
    *,
    ...
    if test_id == "06":
        ...
        if "force_post_activities_next_screen_not_found" not in steps_blob:
            return 0, "FALSE_PASS", "Test 06 missing force_post_activities_next_screen_not_found hook"
...
def run_test_01(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    notes = ["Chain M03 -> M04 -> M06", "Run M20 normal Activities Next discovery"]
    chain = chain_m03_m04_m06(ctx["project"], ctx["matrix_run_id"], "01")
    notes.extend(chain.get("prep_notes", []))
...
def run_test_02(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    notes = ["Chain M03 -> M04 -> M06", "Bring Cursor in front before M20"]
...
def run_test_03(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    notes = ["Chain M03 -> M04 -> M06", "Minimise P6 before M20"]
...
def run_test_04(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    notes = ["Close project with M05", "Run M20 without opening project"]
...
def run_test_05(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    notes = [
        "Chain M03 -> M04 -> M06",
        "Run M20 with force_activities_export_type_not_found",
    ]
...
def run_test_06(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    notes = [
        "Chain M03 -> M04 -> M06",
        "Run M20 with force_post_activities_next_screen_not_found",
    ]
    ...
    m20_result = run_m20(
        ctx["project"],
        evidence=evidence,
        force_post_activities_next_screen_not_found=True,
    )
    return finish_hard_test(test_folder, ctx["test_def"], m20_result, notes)
