"""
M22 — Select Default Activity Template Discovery Only.

Reach template screen via M20 path, detect default/highlighted template, safe focus click
in template list only if needed, then cancel. No path entry or Finish.
"""
...
MODULE_NAME = "m22_select_default_activity_template_discovery_only"
...
def decide_status(
    *,
    ...
    if not default_ok:
        return "FAIL_DEFAULT_TEMPLATE_NOT_FOUND", "Default/highlighted template not confirmed"
    if dialog_closed and default_excerpt:
        return (
            "PASS_DEFAULT_TEMPLATE_DISCOVERY",
            f"Default template confirmed ({default_excerpt[:80]}); wizard closed",
        )
    if dialog_closed:
        return (
            "PASS_DEFAULT_TEMPLATE_DISCOVERY_PARTIAL",
            "Default template confirmed with partial excerpt; wizard closed",
        )
    return "FAIL_DEFAULT_TEMPLATE_NOT_FOUND", "Default template or safe close not confirmed"
...
def write_report(evidence: RunEvidence, result: Dict[str, Any]) -> None:
    lines = [
        "# M22 Default Activity Template Discovery Report",
        "",
        ...
        f"- Default template excerpt: {result.get('default_template_excerpt', '')}",
        f"- Template focus click used: {result.get('template_focus_click_used')}",
        f"- Template focus click text: {result.get('template_focus_click_text', '')}",
...
def run_m22(
    ...
    *,
    ...
    force_default_template_not_found: bool = False,
) -> Dict[str, Any]:
    ...
    try:
        ...
        focus_click_used = bool(focus_text)

        if force_default_template_not_found:
            default_ok = False
            default_excerpt = ""

        save_discovery(
            evidence,
            "default_template_discovery.json",
            {
                ...
                "template_focus_click_used": focus_click_used,
        ...
        closed, close_method, p6_rect = close_export_dialog(
            evidence, p6_keyword, p6_rect, config, screen_rule, template_entries, evidence_words
        )
        ...
        status, reason = decide_status(
            ...
            focus_click_used=focus_click_used,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="M22 Default activity template discovery")
    parser.add_argument("--project", required=True)
    parser.add_argument("--run-id")
    parser.add_argument("--force-default-template-not-found", action="store_true")
    args = parser.parse_args()
    result = run_m22(
        args.project,
        run_id=args.run_id,
        force_default_template_not_found=args.force_default_template_not_found,
    )
