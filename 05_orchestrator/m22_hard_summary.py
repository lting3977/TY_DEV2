...
PASS_DISCOVERY = frozenset(
    {"PASS_DEFAULT_TEMPLATE_NEXT_DISCOVERY", "PASS_DEFAULT_TEMPLATE_NEXT_DISCOVERY_PARTIAL"}
)
...
def write_hard_summary(
    ...
    fixes: List[str] = []
    if export_created or template_action or unsafe:
        fixes.append("Audit M22 default template Next path and template-action guards")
    if false_pass:
    ...
    next_rec = "READY FOR NEXT MODULE" if stable else "FIX M22 AGAIN"
    ...
    summary = {
        ...
        "default_template_evidence": {
            "default_template_detected_count": sum(1 for r in results if r.get("default_template_detected")),
