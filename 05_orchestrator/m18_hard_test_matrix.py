"""
M18 Hard Testing — 6-test matrix.

Proves M18 can safely open the P6 export wizard, select Spreadsheet/XLSX,
press Next exactly once, detect the Export Type screen, cancel safely, and
never create export files.
"""
...
from m18_hard_summary import write_hard_summary  # noqa: E402
...
PASS_DISCOVERY = frozenset(
    {"PASS_SPREADSHEET_NEXT_DISCOVERY", "PASS_SPREADSHEET_NEXT_DISCOVERY_PARTIAL"}
)
TEST_04_OK = frozenset({"FAIL_PROJECT_NOT_OPEN"})
TEST_05_OK = frozenset({"FAIL_SPREADSHEET_OPTION_NOT_FOUND"})
...
def build_test_folder(matrix_run_id: str, test_id: str, slug: str) -> Path:
    folder = (
        ROOT / "06_output" / "runs" / matrix_run_id / "m18_hard_test_6" / f"test_{test_id}_{slug}"
    )
    folder.mkdir(parents=True, exist_ok=True)
    return folder


def build_m18_evidence(matrix_run_id: str, test_id: str, slug: str) -> RunEvidence:
...
def discovery_files_ok(test_folder: Path, require: bool) -> Tuple[bool, bool]:
    ...
    nxt = test_folder / "discovery" / "next_screen_evidence.json"
    if not require:
        return sel.exists() or nxt.exists(), sel.exists() and nxt.exists()
    return sel.exists() and nxt.exists(), sel.exists() and nxt.exists()


def count_next_pressed(m18_result: Dict[str, Any]) -> int:
    return int(m18_result.get("next_pressed_count", 0))


def score_result(
    test_id: str,
    m18_status: str,
    m18_result: Dict[str, Any],
    test_folder: Path,
    *,
    unsafe_ok: bool,
    ...
    export_file = bool(m18_result.get("export_file_created"))
    dialog_closed = bool(m18_result.get("export_dialog_closed"))
    wizard_detected = bool(m18_result.get("export_wizard_detected"))
    next_count = count_next_pressed(m18_result)
    finish_pressed = bool(m18_result.get("finish_pressed"))
    spreadsheet_detected = bool(m18_result.get("spreadsheet_option_detected"))
    next_screen = bool(m18_result.get("next_screen_detected"))
    steps_blob = " ".join(m18_result.get("steps", [])).lower()
    ...
    if test_id == "04":
        if m18_status not in TEST_04_OK:
            return 0, "FALSE_PASS", f"Test 04 expected FAIL_PROJECT_NOT_OPEN, got {m18_status}"
        ...
        return 1, m18_status, "Project not open; export wizard not opened; Next not pressed"

    if test_id == "05":
        if m18_status not in TEST_05_OK:
            return 0, "FALSE_PASS", f"Test 05 expected FAIL_SPREADSHEET_OPTION_NOT_FOUND, got {m18_status}"
        if next_count > 0 or "press next once" in steps_blob:
            return 0, "FALSE_PASS", "Next pressed when Spreadsheet option not confirmed"
        if spreadsheet_detected and "force_spreadsheet_not_found" not in steps_blob:
            return 0, "FALSE_PASS", "Spreadsheet detected without force hook in test 05"
        return 1, m18_status, "Spreadsheet blocked; Next not pressed"

    if test_id == "06":
        if m18_status not in TEST_06_OK:
            return 0, "FALSE_PASS", f"Test 06 expected controlled fail, got {m18_status}"
        if next_count > 1:
            return 0, "NEXT_PRESSED_MORE_THAN_ONCE", "Test 06: Next pressed more than once"
        if "force_next_screen_not_found" not in steps_blob:
            return 0, "FALSE_PASS", "Test 06 missing force_next_screen_not_found hook"
        if next_screen and m18_status == "FAIL_NEXT_SCREEN_NOT_FOUND":
            pass
        elif m18_status == "PASS_SPREADSHEET_NEXT_DISCOVERY":
            return 0, "FALSE_PASS", "Test 06 should not full PASS with forced next-screen block"
        return 1, m18_status, "Controlled next-screen failure; Finish not pressed"

    if test_id == "03":
        if m18_status == "FAIL_P6_WINDOW_NOT_READY":
            if export_file or wizard_detected or next_count > 0:
                return 0, "FALSE_PASS", "Export attempted when P6 window not ready"
            return 1, m18_status, "P6 could not safely restore; no export attempted"
        if m18_status not in PASS_DISCOVERY:
            return 0, "FALSE_PASS", (
                f"Test 03 expected discovery pass or FAIL_P6_WINDOW_NOT_READY, got {m18_status}"
            )
    ...
    if m18_status not in PASS_DISCOVERY:
        return 0, "FALSE_PASS", (
            f"Expected PASS_SPREADSHEET_NEXT_DISCOVERY or PARTIAL, got {m18_status}"
        )
    ...
    disc_ok, _ = discovery_files_ok(test_folder, m18_status in PASS_DISCOVERY)
    if m18_status in PASS_DISCOVERY and not disc_ok:
        return 0, "FALSE_PASS", "discovery JSON files missing for successful test"

    after_state = (m18_result.get("screen_state_after") or "").lower()
    title_after = (m18_result.get("window_title_after") or "").lower()
    ...
    return (
        1,
        m18_status,
        f"Spreadsheet next-screen OK; next_count={next_count}; closed={dialog_closed}",
    )
...
def finish_hard_test(
    ...
    m18_status = m18_result.get("status", "ERROR")
    unsafe_ok, unsafe_hits = check_unsafe_steps(m18_result.get("steps", []))
    crop_ok, crop_issues = check_no_fullscreen_ocr(test_folder)
    pollution_ok, pollution_hits = check_ocr_pollution(test_folder)
    format_ok, format_hits = check_wrong_format(m18_result.get("steps", []))
    require_disc = m18_status in PASS_DISCOVERY
    ...
    result = {
        ...
        "slug": test_def["slug"],
        "m18_run_id": m18_result.get("run_id", ""),
        "m18_status": m18_status,
        "m18_reason": m18_result.get("reason", ""),
        "score": score,
        "status": status,
        "score_reason": score_reason,
        "window_title_before": m18_result.get("window_title_before", ""),
        "window_title_after": m18_result.get("window_title_after", ""),
        "screen_state_before": m18_result.get("screen_state_before", ""),
        "screen_state_after": m18_result.get("screen_state_after", ""),
        "export_wizard_detected": m18_result.get("export_wizard_detected"),
        "spreadsheet_option_detected": m18_result.get("spreadsheet_option_detected"),
        "spreadsheet_option_text": m18_result.get("spreadsheet_option_text", ""),
        "spreadsheet_option_selected": m18_result.get("spreadsheet_option_selected"),
        "next_button_detected": m18_result.get("next_button_detected"),
        "next_pressed_count": m18_result.get("next_pressed_count", 0),
        "finish_button_detected": m18_result.get("finish_button_detected"),
        "finish_pressed": m18_result.get("finish_pressed"),
        "next_screen_detected": m18_result.get("next_screen_detected"),
        "next_screen_evidence_words": m18_result.get("next_screen_evidence_words", []),
        "cancel_button_detected": m18_result.get("cancel_button_detected"),
        "export_dialog_closed": m18_result.get("export_dialog_closed"),
        "close_method_used": m18_result.get("close_method_used", ""),
        "export_file_created": m18_result.get("export_file_created"),
    ...
    lines = [
        f"# M18 Hard Test {test_def['id']} — {test_def['name']}",
        "",
        f"- M18 run ID: {m18_result.get('run_id', '')}",
        f"- M18 status: {m18_status}",
        ...
        f"- Next screen detected: {result['next_screen_detected']}",
        f"- Next screen evidence words: {result['next_screen_evidence_words']}",
...
def run_test_05(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    notes = [
        "Chain M03 -> M04 -> M06",
        "Run M18 with force_spreadsheet_not_found (Spreadsheet blocked)",
    ]
    chain = chain_m03_m04_m06(ctx["project"], ctx["matrix_run_id"], "05")
    notes.append(f"M06 chain status: {chain['m06'].get('status')}")
    evidence = build_m18_evidence(ctx["matrix_run_id"], "05", ctx["test_def"]["slug"])
    m18_result = run_m18(
        ctx["project"],
        evidence=evidence,
        force_spreadsheet_not_found=True,
    )
    return finish_hard_test(test_folder, ctx["test_def"], m18_result, notes)
...
def run_test_06(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    notes = [
        "Chain M03 -> M04 -> M06",
        "Run M18 with force_next_screen_not_found (next screen blocked after Next)",
    ]
    chain = chain_m03_m04_m06(ctx["project"], ctx["matrix_run_id"], "06")
    notes.append(f"M06 chain status: {chain['m06'].get('status')}")
    evidence = build_m18_evidence(ctx["matrix_run_id"], "06", ctx["test_def"]["slug"])
    m18_result = run_m18(
        ctx["project"],
        evidence=evidence,
        force_next_screen_not_found=True,
    )
    return finish_hard_test(test_folder, ctx["test_def"], m18_result, notes)
...
HARD_TESTS: List[Dict[str, Any]] = [
    {
        "id": "01",
        "slug": "normal_spreadsheet_next_screen_discovery",
        "name": "Normal Spreadsheet next-screen discovery",
        "runner": run_test_01,
    },
    {
        "id": "02",
    ...
    {
        "id": "05",
        "slug": "spreadsheet_option_blocked",
        "name": "Spreadsheet option missing / blocked condition",
        "runner": run_test_05,
    },
    {
        "id": "06",
        "slug": "next_screen_blocked",
        "name": "Next-screen blocked / unclear condition",
        "runner": run_test_06,
    },
]
...
def run_matrix(project: str) -> Dict[str, Any]:
    ...
    (run_root / "m18_hard_test_6").mkdir(parents=True, exist_ok=True)

    print("M18 Hard Testing — 6-test matrix")
    ...
    print(f"Summary: {run_root / 'm18_hard_test_6_summary.json'}")
    return summary


def main() -> int:
    parser = argparse.ArgumentParser(description="M18 Hard Testing 6-test matrix")
    parser.add_argument("--project", required=True, help='Project name e.g. "Talison 1275"')
    args = parser.parse_args()
    summary = run_matrix(args.project.strip())
    if summary.get("decision") == "M18 STABLE":
        return 0
    return 1
