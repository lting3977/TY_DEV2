"""Build M14 hard 6-test matrix summary."""
...
def write_hard_summary(
    ...
    clipboard_success = [
        r
        for r in results
        if r.get("m14_status") in PASS_CLIPBOARD and int(r.get("score", 0)) == 1
    ]
    ...
    if pollution_cases or fg_not_confirmed or grid_outside:
        fixes.append("Audit M14 foreground confirmation and row target selection")
    if not_restored:
        fixes.append("Verify clipboard restore path after copy failures")
    if false_pass:
        fixes.append("Tighten M14 hard-test scoring gates")
    if not fixes:
        fixes.append("Monitor with TY_TEST_M14_HARD_6.bat")
        fixes.append("Human sign-off before next module")
        fixes.append("Keep M14 read-only multi-select constraints enabled")
    while len(fixes) < 3:
        fixes.append("Re-run hard matrix after P6 layout changes")

    decision = "M14 STABLE" if stable else "M14 NEEDS FIX"
    next_rec = "READY FOR NEXT MODULE" if stable else "FIX M14 AGAIN"

    pass_count = sum(1 for r in clipboard_success if r.get("m14_status") == "PASS")
    partial_count = sum(
        1 for r in clipboard_success if r.get("m14_status") == "PASS_PARTIAL_CLIPBOARD"
    )
    ...
    selection_methods = sorted(
        {r.get("selection_method_used", "") for r in results if r.get("selection_method_used")}
    )
    ...
    json_path = run_root / "m14_hard_test_6_summary.json"
    with json_path.open("w", encoding="utf-8") as handle:
        json.dump(summary, handle, indent=2, ensure_ascii=False)

    md_lines = [
        "# M14 Hard Testing Summary",
        "",
        f"- Run ID: {run_id}",
        f"- Project: {project}",
    ...
    for r in results:
        md_lines.append(
            ...
            f"m14={r.get('m14_status', '')} reason={r.get('score_reason', '')}"
        )
    md_lines.extend(
        [
            "",
            ...
            f"- Selection methods used: {selection_methods}",
    ...
    md_lines.extend(
        [
            ...
            "## Evidence",
            str(json_path),
            str(run_root / "m14_hard_test_6_summary.md"),
        ]
    )
    md_path = run_root / "m14_hard_test_6_summary.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
