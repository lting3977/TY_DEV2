...
from m22_select_project_on_projects_to_export_discovery_only import (  # noqa: E402
    RunEvidence,
    run_m22,
    write_json,
)
...
PASS_DISCOVERY = frozenset(
    {
        "PASS_PROJECT_SELECTION_NEXT_DISCOVERY",
        "PASS_PROJECT_SELECTION_NEXT_DISCOVERY_PARTIAL",
    }
)
...
TEST_06_OK = frozenset(
    {
        "FAIL_POST_PROJECT_SELECTION_SCREEN_NOT_FOUND",
...
def build_test_folder_m22(matrix_run_id: str, test_id: str, slug: str) -> Path:
    folder = ROOT / "06_output" / "runs" / matrix_run_id / "m22_hard_test_6" / f"test_{test_id}_{slug}"
    folder.mkdir(parents=True, exist_ok=True)
    return folder


def build_m22_evidence_v2(matrix_run_id: str, test_id: str, slug: str) -> RunEvidence:
    folder = build_test_folder_m22(matrix_run_id, test_id, slug)
...
def write_test_report(test_folder: Path, result: Dict[str, Any]) -> None:
    lines = [
        f"# M22 Hard Test {result.get('test_id')} — {result.get('test_name')}",
        "",
        f"- Score: {result.get('score')}",
        f"- Status: {result.get('status')}",
        f"- M22 status: {result.get('m22_status')}",
        f"- Reason: {result.get('score_reason', result.get('m22_reason', ''))}",
        "",
        "## Setup notes",
    ]
...
def run_m22_precheck(
    ...
    *,
    ...
    for try_i in range(tries):
        ...
        last = ensure_clean_p6_for_m22_hard(project, run_label)
...
def score_result(
    ...
    *,
    ...
    post_ok = bool(m22_result.get("post_project_selection_next_screen_detected"))
    ...
    if m22_status in ("SETUP_FAILURE_PYAUTOGUI_FAILSAFE", "SETUP_FAILURE_UNSAFE_CLICK_POINT"):
        return 0, m22_status, m22_result.get("reason", m22_status)

    if test_id == "04":
        if m22_result.get("test04_project_still_open") and m22_status in PASS_DISCOVERY:
    ...
    if test_id == "05":
        ...
        if m22_status not in TEST_05_OK:
            return 0, "FALSE_PASS", f"Test 05 unexpected status: {m22_status}"
        if not projects_ok:
        ...
        return 1, m22_status, "Project row not found at expected hook stage"

    if test_id == "06":
        if not hook_stage_ok_test06(forced) and not forced.get("hook_applied_after_project_selection_next"):
            return 0, "SETUP_FAILURE", "Test 06 hook not applied after project selection Next"
        if not hook_stage_ok_test06(forced):
            return 0, "SETUP_FAILURE", "Test 06 missing required M22 wizard stage evidence"
        if m22_status in PASS_DISCOVERY:
            return 0, "FALSE_PASS", "Test 06 should not PASS with forced post-selection block"
        if m22_status not in TEST_06_OK:
            return 0, "FALSE_PASS", f"Test 06 expected controlled fail, got {m22_status}"
        if finish_pressed or export_file:
            return 0, "UNSAFE_ACTION", "Test 06 unsafe action detected"
        return 1, m22_status, "Controlled post-project-selection failure; Finish not pressed"

    if test_id == "03" and m22_status == "FAIL_P6_WINDOW_NOT_READY":
    ...
    if test_id == "03" and m22_status in PASS_DISCOVERY:
        return 1, m22_status, "P6 minimised restore path succeeded"

    if test_id == "02" and not pollution_ok and m22_status in PASS_DISCOVERY:
        return 0, "FALSE_PASS", f"OCR pollution: {pollution_hits[:3]}"

    if m22_status not in PASS_DISCOVERY:
        return 0, "FALSE_PASS", f"Expected project selection discovery PASS, got {m22_status}"
    ...
    if not post_ok and m22_status == "PASS_PROJECT_SELECTION_NEXT_DISCOVERY":
    ...
    return 1, m22_status, f"M22 project selection discovery OK; next_count={next_count}; closed={dialog_closed}"
...
def finish_hard_test(
    ...
    result = {
        ...
        "post_project_selection_next_screen_detected": m22_result.get("post_project_selection_next_screen_detected"),
        "post_project_selection_screen_type": m22_result.get("post_project_selection_screen_type", ""),
        "post_project_selection_evidence_words": m22_result.get("post_project_selection_evidence_words", []),
...
def precheck_or_fail(
    ...
    ok, notes, precheck = run_m22_precheck(
        ctx["project"],
        ctx["matrix_run_id"],
        test_folder,
        label,
...
def run_m22_after_hard_precheck(
    ctx: Dict[str, Any],
    evidence: RunEvidence,
    **kwargs: Any,
) -> Dict[str, Any]:
    """Hard matrix precheck already restored P6; skip redundant M22 clean-restore."""
    return run_m22(ctx["project"], evidence=evidence, skip_project_restore=True, **kwargs)
...
def run_test_01(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    ...
    if fail:
        return fail
    evidence = build_m22_evidence_v2(ctx["matrix_run_id"], "01", ctx["test_def"]["slug"])
    return finish_hard_test(test_folder, ctx["test_def"], run_m22_after_hard_precheck(ctx, evidence), notes)
...
def run_test_02(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    ...
    evidence = build_m22_evidence_v2(ctx["matrix_run_id"], "02", ctx["test_def"]["slug"])
    return finish_hard_test(test_folder, ctx["test_def"], run_m22_after_hard_precheck(ctx, evidence), notes)
...
def run_test_03(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    ...
    time.sleep(0.5)
    evidence = build_m22_evidence_v2(ctx["matrix_run_id"], "03", ctx["test_def"]["slug"])
    return finish_hard_test(test_folder, ctx["test_def"], run_m22_after_hard_precheck(ctx, evidence), notes)
...
def run_test_04(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    ...
    notes.append(f"project_still_open_before_m22={project_still_open}")
    evidence = build_m22_evidence_v2(ctx["matrix_run_id"], "04", ctx["test_def"]["slug"])
    m22_result = run_m22(ctx["project"], evidence=evidence, skip_project_restore=True)
    m22_result["test04_project_still_open"] = project_still_open
    result = finish_hard_test(test_folder, ctx["test_def"], m22_result, notes)
    restore = ensure_clean_p6_for_m22_hard(ctx["project"], f"{ctx['matrix_run_id']}_t04_post_restore")
...
def run_test_05(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    ...
    if fail:
        return fail
    evidence = build_m22_evidence_v2(ctx["matrix_run_id"], "05", ctx["test_def"]["slug"])
    m22_result = run_m22_after_hard_precheck(
        ctx,
        evidence,
...
def run_test_06(ctx: Dict[str, Any], test_folder: Path) -> Dict[str, Any]:
    ...
    if fail:
        return fail
    evidence = build_m22_evidence_v2(ctx["matrix_run_id"], "06", ctx["test_def"]["slug"])
    m22_result = run_m22_after_hard_precheck(
        ctx,
        evidence,
        force_post_project_selection_screen_not_found=True,
    )
    import json
...
HARD_TESTS: List[Dict[str, Any]] = [
    {"id": "01", "slug": "normal_project_selection_discovery", "name": "Normal project selection to template", "runner": run_test_01},
    ...
    {"id": "06", "slug": "post_project_selection_screen_blocked", "name": "Post-project-selection screen blocked", "runner": run_test_06},
]


def run_baseline_precheck(project: str, matrix_run_id: str, run_root: Path) -> Tuple[bool, Dict[str, Any]]:
    baseline = ensure_clean_p6_for_m22_hard(project, f"{matrix_run_id}_baseline")
    write_json(run_root / "baseline_restore.json", baseline)
    md_lines = [
        "# M22 Hard Test Baseline Restore",
        "",
...
def run_matrix(project: str) -> Dict[str, Any]:
    ...
    if not baseline_ok:
        ...
        write_json(run_root / "m22_hard_test_6_summary.json", summary)
    ...
    for index, test_def in enumerate(HARD_TESTS, start=1):
        ...
        test_folder = build_test_folder_m22(matrix_run_id, test_def["id"], test_def["slug"])
        ...
        try:
            ...
            result = {
                ...
                "test_name": test_def["name"],
                "slug": test_def["slug"],
                "m22_status": "SETUP_FAILURE_PYAUTOGUI_FAILSAFE",
                "score": 0,
