"""
M22 — Select Project on Projects-to-export Discovery Only.

Spreadsheet -> Export Type -> Activities -> Projects-to-export ->
select 001 Talison 1275 -> Next once -> inspect next wizard screen, then cancel safely.
...
from eye.ocr import collect_text_blob, is_easyocr_available  # noqa: E402
...
from export_wizard_common import (  # noqa: E402
    ...
    m22_controlled_wizard_to_post_project_selection_next,
...
MODULE_NAME = "m22_select_project_on_projects_to_export_discovery_only"
...
def decide_status(
    *,
    ...
    if browse_clicked or path_typed or template_modified:
    ...
    if not project_row_detected or not project_selection_attempted:
        return "FAIL_PROJECT_ROW_NOT_FOUND", "Project row selection not attempted"
    if validation_after_selection or post_screen_type == "projects_validation_popup":
        return "FAIL_PROJECT_SELECTION_NOT_CONFIRMED", "Validation popup after project selection Next"
    if next_pressed_count < 3:
        return "FAIL_POST_PROJECT_SELECTION_SCREEN_NOT_FOUND", "Third Next after project select not confirmed"
    if post_class_status == "FAIL_PROJECT_SELECTION_NOT_CONFIRMED":
        return post_class_status, post_class_reason or "Project selection not confirmed"
    if post_class_status == "FAIL_POST_PROJECT_SELECTION_SCREEN_NOT_FOUND":
        return post_class_status, post_class_reason or "Post-project-selection screen not found"
    if not post_screen_ok:
        return "FAIL_POST_PROJECT_SELECTION_SCREEN_NOT_FOUND", "Post-project-selection next screen not confirmed"
    ...
    if post_class_status in PASS_STATUSES:
        return post_class_status, post_class_reason or f"Post-project-selection {post_screen_type} discovered"
    if post_screen_type == "template" and (dialog_closed or cleanup_success):
        return "PASS_PROJECT_SELECTION_NEXT_DISCOVERY", "Template screen after project selection Next"
    if post_screen_type in ("file_path", "generic_wizard") and (dialog_closed or cleanup_success):
        if post_screen_type == "generic_wizard":
            return (
                "PASS_PROJECT_SELECTION_NEXT_DISCOVERY_PARTIAL",
                "Partial post-project-selection wizard discovery; wizard closed safely",
            )
        return "PASS_PROJECT_SELECTION_NEXT_DISCOVERY", "File/path screen after project selection Next"
    return "FAIL_POST_PROJECT_SELECTION_SCREEN_NOT_FOUND", "Post-project-selection evidence or safe close not confirmed"
...
def write_report(evidence: RunEvidence, result: Dict[str, Any]) -> None:
    lines = [
        "# M22 Select Project on Projects-to-export Discovery Report",
        "",
        f"- Run ID: {result['run_id']}",
        f"- Project: {result.get('project_name', '')}",
        f"- Status: {result['status']}",
        f"- Reason: {result['reason']}",
        f"- Project row text: {result.get('project_row_text', '')}",
        f"- Project row selected: {result.get('project_row_selected')}",
        f"- Next pressed total: {result.get('next_pressed_count_total')}",
        f"- Post-selection screen type: {result.get('post_project_selection_screen_type', '')}",
        f"- Validation popup after selection: {result.get('validation_popup_detected_after_project_selection')}",
...
def _base_result_fields(
    *,
    ...
    return {
        ...
        "post_project_selection_next_screen_detected": bool(ctx.get("post_project_selection_screen_ok")),
        "post_project_selection_screen_type": ctx.get("post_project_selection_screen_type", ""),
        "post_project_selection_evidence_words": ctx.get("post_project_selection_evidence_words", []),
        "validation_popup_detected_after_project_selection": bool(
            ctx.get("validation_popup_detected_after_project_selection")
        ),
        ...
        "finish_button_detected": ctx.get("finish_button_detected", False),
...
def run_m22(
    ...
    *,
    ...
    def timed_out() -> bool:
        return (time.monotonic() - timer_start) > M22_MAX_RUN_SEC
    ...
    try:
        if skip_project_restore:
            ...
            clean = ensure_clean_p6_for_m21_hard(project_name, f"{evidence.run_id}_m22_clean")
            preflight_meta = {
        ...
        if prep_err and prep_err.get("status") == "MANUAL_REVIEW_CANNOT_CONFIRM" and prep_err.get(
            "manual_review_required"
        ):
            evidence.steps.append("M22: dirty preflight pollution — retry ensure_clean once")
            clean2 = ensure_clean_p6_for_m21_hard(project_name, f"{evidence.run_id}_m22_clean_retry")
            preflight_meta["project_restore_attempts"] = preflight_meta.get("project_restore_attempts", 0) + len(
                clean2.get("attempts", [])
            )
            if clean2.get("ok"):
                p6_rect, window_title_before, screen_state_before, dirty_meta, prep_err = (
        ...
        if timed_out():
            ...
            return finish_result(
                ...
                f"M22 exceeded {M22_MAX_RUN_SEC}s after preflight",
                **_base_result_fields(
                    window_title_before=window_title_before,
                    screen_state_before=screen_state_before,
                    preflight_meta=preflight_meta,
        ...
        p6_rect, ctx, path_err = m22_controlled_wizard_to_post_project_selection_next(
            evidence,
            p6_keyword,
            p6_rect,
            config,
            ...
            min_confidence,
            project_name,
            force_skip_project_row_select=force_skip_project_row_select,
            force_project_row_not_found=force_project_row_not_found,
            force_post_project_selection_screen_not_found=force_post_project_selection_screen_not_found,
        )
        ...
        post_ok = bool(ctx.get("post_project_selection_screen_ok", False))
        post_class_status = ctx.get("post_project_selection_classification_status", "")
        post_class_reason = ctx.get("post_project_selection_classification_reason", "")
        validation_after = bool(ctx.get("validation_popup_detected_after_project_selection"))

        if post_type == "projects_validation_popup" or validation_after:
            ctx["validation_popup_detected_after_project_selection"] = True
            dismissed, _, p6_rect = m21_dismiss_projects_validation_popup(
                evidence,
                p6_rect,
                p6_keyword,
                config,
        ...
        if not safe_steps:
            status, reason = "MANUAL_REVIEW_UNSAFE_POPUP", "Unsafe step detected in M22 run"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="M22 select project on Projects-to-export discovery")
    parser.add_argument("--project", required=True)
    parser.add_argument("--run-id")
    args = parser.parse_args()
    result = run_m22(args.project, run_id=args.run_id)
    print(f"M22 status: {result['status']}")
