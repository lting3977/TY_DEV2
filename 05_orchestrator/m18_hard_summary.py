"""Build M18 hard 6-test matrix summary."""
...
def write_hard_summary(
    ...
    discovery_success = [
        r
        for r in results
        if r.get("m18_status") in PASS_DISCOVERY and int(r.get("score", 0)) == 1
    ]
    ...
    if export_created or finish_pressed or next_multi or unsafe:
        fixes.append("Audit M18 Spreadsheet discovery close path and Next/Finish guards")
    if dialog_open:
        fixes.append("Verify Esc/Cancel closes export wizard completely after Next screen")
    if wrong_format:
        fixes.append("Tighten Spreadsheet-only OCR click targeting")
    if false_pass:
        fixes.append("Tighten M18 hard-test scoring gates")
    if not fixes:
        fixes.append("Monitor with TY_TEST_M18_HARD_6.bat")
        fixes.append("Human sign-off before next module")
        fixes.append("Keep M18 discovery-only constraints enabled")
    while len(fixes) < 3:
        fixes.append("Re-run hard matrix after P6 layout changes")

    decision = "M18 STABLE" if stable else "M18 NEEDS FIX"
    next_rec = "READY FOR NEXT MODULE" if stable else "FIX M18 AGAIN"

    pass_full = sum(
        1 for r in discovery_success if r.get("m18_status") == "PASS_SPREADSHEET_NEXT_DISCOVERY"
    )
    pass_partial = sum(
        1
        for r in discovery_success
        if r.get("m18_status") == "PASS_SPREADSHEET_NEXT_DISCOVERY_PARTIAL"
    )

    text_examples: List[str] = []
    next_counts: List[int] = []
    next_words_union: List[str] = []
    for r in discovery_success:
        ...
        for w in r.get("next_screen_evidence_words", []) or []:
            if w not in next_words_union:
                next_words_union.append(w)
    ...
    next_screen_count = sum(1 for r in results if r.get("next_screen_detected"))
    ...
    summary = {
        ...
        "spreadsheet_next_screen_evidence": {
            "successful_discovery_tests": len(discovery_success),
            "pass_spreadsheet_next_discovery_count": pass_full,
            "pass_spreadsheet_next_discovery_partial_count": pass_partial,
            ...
            "next_screen_detected_count": next_screen_count,
            "next_screen_evidence_words": next_words_union,
    ...
    json_path = run_root / "m18_hard_test_6_summary.json"
    json_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")

    de = summary["spreadsheet_next_screen_evidence"]
    md_lines = [
        "# M18 Hard Testing Summary",
        "",
        f"- Run ID: {run_id}",
        f"- Project: {project}",
    ...
    for r in results:
        md_lines.append(
            ...
            f"status={r.get('status')} m18={r.get('m18_status')} reason={r.get('score_reason', '')}"
        )

    md_lines.extend(
        [
            "",
            "## Spreadsheet next-screen discovery evidence",
            f"- Successful discovery tests: {de['successful_discovery_tests']}",
            f"- PASS_SPREADSHEET_NEXT_DISCOVERY count: {de['pass_spreadsheet_next_discovery_count']}",
            f"- PASS_SPREADSHEET_NEXT_DISCOVERY_PARTIAL count: {de['pass_spreadsheet_next_discovery_partial_count']}",
            ...
            f"- Next screen detected: {de['next_screen_detected_count']}",
            f"- Next screen evidence words: {de['next_screen_evidence_words']}",
    ...
    md_lines.extend(
        [
            ...
            "## Evidence",
            str(json_path),
            str(run_root / "m18_hard_test_6_summary.md"),
        ]
    )
    md_path = run_root / "m18_hard_test_6_summary.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    return summary
