"""Build M23 hard 6-test matrix summary."""
...
def write_hard_summary(
    ...
    if not fixes:
        fixes.append("Monitor with TY_TEST_M23_HARD_6.bat")
        fixes.append("Human sign-off before next module")
        fixes.append("Keep M23 discovery-only constraints enabled")
    while len(fixes) < 3:
    ...
    json_path = run_root / "m23_hard_test_6_summary.json"
    json_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
    md_lines = [
        "# M23 Hard Testing Summary",
        "",
        f"- Run ID: {run_id}",
        f"- Project: {project}",
    ...
    for r in results:
        md_lines.append(
            ...
            f"status={r.get('status')} m23={r.get('m23_status')}"
        )
    md_lines.extend(["", "## Next recommendation", next_rec, "", "## Evidence", str(json_path)])
    (run_root / "m23_hard_test_6_summary.md").write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    return summary
