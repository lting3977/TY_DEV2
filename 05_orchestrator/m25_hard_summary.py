"""Build M25 hard 6-test matrix summary."""
...
def write_hard_summary(
    ...
    fixes: List[str] = []
    if false_pass:
        fixes.append("Tighten M25 sandbox guard validation gates")
    if not fixes:
        fixes.append("Monitor with TY_TEST_M25_HARD_6.bat")
        fixes.append("Human sign-off before next module")
        fixes.append("Keep sandbox root under 06_output/exports/sandbox")
    while len(fixes) < 3:
        fixes.append("Re-run hard matrix after sandbox policy changes")

    decision = "M25 STABLE" if stable else "M25 NEEDS FIX"
    next_rec = "READY FOR NEXT MODULE" if stable else "FIX M25 AGAIN"
    ...
    json_path = run_root / "m25_hard_test_6_summary.json"
    json_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")

    md_lines = [
        "# M25 Hard Testing Summary",
        "",
        f"- Run ID: {run_id}",
        f"- Project: {project}",
    ...
    for r in results:
        md_lines.append(
            ...
            f"status={r.get('status')} m25={r.get('m25_status')} reason={r.get('score_reason', '')}"
        )
    ...
    md_path = run_root / "m25_hard_test_6_summary.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    return summary
